...
__all__ = ["load_shapenet_json"]


def load_shapenet_json(json_file, image_root, dataset_name=None):
    """
    Load a json file with Shapenet's instances annotation format.
...
"""
    ...
    for (img_dict, annotation_dict_list) in imgs_anns:
        ...
        for annotation in annotation_dict_list:
            ...
            assert annotation.get("ignore", 0) == 0

            obj = {
                field: annotation[field] for field in ["iscrowd", "bbox", "category_id"] if field in annotation
            }

            segm = annotation.get("segmentation", None)
            if segm:  # string
                obj["segmentation"] = os.path.join(image_root, segm)

            voxel = annotation.get("voxel", None)
            if voxel:
                obj["voxel"] = os.path.join(image_root, voxel)

            mesh = annotation.get("model", None)
            if mesh:
                obj["mesh"] = mesh

            pointcloud = annotation.get("pointcloud", None)
            if pointcloud:
            ...
            # camera
            obj["K"] = annotation["K"]
            obj["R"] = annotation["rot_mat"]
            obj["t"] = annotation["trans_mat"]
...
#     """
#     Test the ShapeNet json dataset loader.
#
#     Usage:
#         python -m meshrcnn.data.datasets.shapenet \
